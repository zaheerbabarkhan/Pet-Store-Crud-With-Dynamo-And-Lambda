...
// src/functions/PetCRUD/handler.ts
__export(exports, {
  addPet: () => addPet,
  deletePet: () => deletePet,
  findPet: () => findPet,
  findPetWithTags: () => findPetWithTags,
  updatePet: () => updatePet
});
...
var add = async (event) => {
  ...
  const response = await dynamo_default.savePet(petData, "PetPetTable1");
  const response2 = await dynamo_default.getalldata("PetPetTable1");
  return formatJSONResponse({
    Pet: response,
    allPets: response2
  });
};
var addPet = middyfy(add);
var find = async (event) => {
  const { id } = event.pathParameters;
  console.log(id);
  const response = await dynamo_default.getPet(id, "PetPetTable1");
  return formatJSONResponse({
    Pet: response
  });
};
var findPet = middyfy(find);
var deletepet = async (event) => {
  const { id } = event.pathParameters;
  console.log(id);
  const response = await dynamo_default.deletePet(id, "PetPetTable1");
  return formatJSONResponse({
    Pet: response
  });
};
var deletePet = middyfy(deletepet);
var findWithTags = async (event) => {
  const { limit, tags } = event.queryStringParameters;
  const response = await dynamo_default.getWithTags(limit, tags, "PetPetTable1");
  return formatJSONResponse({
    Pet: response
  });
};
var findPetWithTags = middyfy(findWithTags);
var updateHandler = async (event) => {
  const updateData = event.body;
  const response = await dynamo_default.updatePet(updateData, "PetPetTable1");
  return formatJSONResponse({
    response
  });
};
var updatePet = middyfy(updateHandler);
